...
export function BasicDataTable() {
  ...
  const FetchcouponList = async () => {
    try {
      const apiResData = await getApiData(`coupon/list`);
      if (apiResData.success === true) {
        setData(apiResData?.coupons);
      } else {
        setData([]);
  ...
  const couponDelete = async () => {
    toast.dismiss();
    try {
      const response = await deleteApiData(`coupon/${_id}`);
 ...
 const ViewConfirm = async (_id) => {
  try {
    const apiResData = await getApiData(`coupon/${_id}`);
    if (apiResData) {
      setView(apiResData?.coupon);
  ...
  const columns = [
    ...
    {
      accessorKey: "active",
      header: "Coupon Active",
      cell: ({ row }) => {
        const formattedDate = new Date(row.original.active).toLocaleDateString();
        return (
          <div className="whitespace-nowrap">
    ...
    {
      accessorKey: "Expired",
      header: "Coupon Expired",
      cell: ({ row }) => {
        const formattedDate = new Date(row.original.expired).toLocaleDateString();
        return (
          <div className="whitespace-nowrap">
            <span>{formattedDate}</span>
          </div>
   ...
   
    {
      ...
      cell: ({ row }) => (
        <div className="flex space-x-3 rtl:space-x-reverse">
          <Link href={`/admin/coupon/edit/${row?.original?._id}`}>
            <Button
              size="icon"
              variant="outline"
              color="secondary"
        ...
        <Dialog open={isOpen} onOpenChange={handleClose}>
          ...
          <DialogContent>
            ...
            <DialogFooter className="mt-8 gap-2">
              ...
              <Button onClick={() => couponDelete()} type="button">
                Delete Confirm
              </Button>
              {/* </Link> */}
            </DialogFooter>
