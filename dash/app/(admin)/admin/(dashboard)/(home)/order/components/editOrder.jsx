...
import { getApiData, postApiData, updateApiData, updateApiFormData } from "@/helper/common";
...
const Form = () => {
  ...
  const ViewConfirm = async () => {
    try {
      const apiResData = await getApiData(`orders/${params?.id}`);
      if (apiResData) {
        setView(apiResData?.order);
        setStatus(apiResData?.order?.status)
        setpayment_status(apiResData?.order?.payment_status)
  ...
  const handleOrderSubmit = async (event) => {
    ...
    try {
      ...
      const response = await updateApiData(`orders/${params?.id}`, apiData);
